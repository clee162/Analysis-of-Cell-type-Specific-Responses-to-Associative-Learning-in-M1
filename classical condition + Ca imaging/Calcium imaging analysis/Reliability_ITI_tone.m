...
for d = 1:num_days
    ...
    for mouse = 1:n
        ...
        wt_start = [];
        real_rois = [];
        num_tone_responses_daily = [];   %number of trials an ROI responds to on a given day. each row is an ROI
...
%             df(any(isnan(df),2),:) = [];
        ...
        % a trial
        [tone_responsive_ROI_indices, ~] = find_indices_of_tone_active_ROIs_v3(df_z, tone_start, window);
        tone_responsive_ROI_ind_mat = cell2mat(tone_responsive_ROI_indices); 
        ...
        for curr_roi = 1:num_rois
            num_tone_responses_daily(curr_roi) = length(find(tone_responsive_ROI_ind_mat == curr_roi));   %number of trials an ROI responds to on a given day. each row is an ROI
            num_ITI_responses_daily(curr_roi) = length(find(ITI_responsive_ROI_ind_mat == curr_roi));
        end
        
        num_tone_responses{mouse,d} = num_tone_responses_daily;
        num_ITI_responses{mouse,d} = num_ITI_responses_daily;
       
        percent_tone_responses(mouse,d) = mean((num_tone_responses_daily./num_trials_all(mouse,d)).*100);
...
SEM_tone = (std(percent_tone_responses,0,1))./sqrt(n);

overall_mean_ITI = mean(percent_ITI_responses,1);
overall_mean_tone = mean(percent_tone_responses,1);
...
for d = 1:num_days 
    ...
    for mouse = 1:n 
        plot(x,[percent_ITI_responses(mouse,d) percent_tone_responses(mouse,d)],'-o',...
            'Color', c1,...
            'MarkerFaceColor', c1)
        hold on
    end
    e = errorbar(x, [overall_mean_ITI(d) overall_mean_tone(d)], [SEM_ITI(d) SEM_tone(d)], '-o', 'MarkerSize', 4,...
    'MarkerFaceColor', c2,... 
    'MarkerEdgeColor', c2,... 
    'LineWidth', 1.5);
    e.Color = c2;
    ...
    xticks([1 2])
    ylim([0 50])
    xticklabels({'ITI','Tone'})
    title(days(d))
    [h,p(d)] = ttest(percent_ITI_responses(:,d), percent_tone_responses(:,d));
...
%% save
file_name_e = strcat(celltype,'_ITI_vs_tone.eps'); % .eps OR .tif
file_name_t = strcat(celltype,'_ITI_vs_tone.tif'); % .eps OR .tif
...
for d = 1:2
    ...
    if d == 1
        ...
        R_s = strcat(d1_R, num2str(round(overall_mean_reward(d),2)),{'+/-'}, num2str(round(SEM_tone(d),2)),'%, ');
    ...
    if d == 2
        ...
        R_s = strcat(d7_R, num2str(round(overall_mean_reward(d),2)),{'+/-'}, num2str(round(SEM_tone(d),2)),'%');
        p_s = strcat({'p = '}, num2str(round(p(d),2)));
        disp(strcat(ITI_s, R_s, p_s))
    end
end
